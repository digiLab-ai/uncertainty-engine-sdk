...
def validate_outputs_exist(node_info: NodeInfo, node_outputs: dict[str, Any]):
    """
    ...
    Args:
        node_info: A `NodeInfo` object used to validate the node outputs
            against.
        node_outputs: A dictionary containing the node outputs.
    ...
    if invalid_output_names:
        raise NodeValidationError(f"Invalid input names: {invalid_output_names}")
